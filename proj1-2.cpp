...
class Component// We have this as an interface(Component) for creating composite pattern and also this as an observer.
{
private:
...
public:
  ...
  virtual void update(){}// We display updates from this.
...
public:
  ...
  virtual void update(){// We use this function for printing out the information gotten by ListServers.
...
private: std::vector<Composite*> comps;
         std::string Name;
public:
    Observable(std::string s){
    Name = s;
    cout<<"\n Created a new ListServer: "<<Name<<endl;
    }
    virtual void attach(Composite* coms){
        comps.push_back(coms);
    }// We attach observers which we want to keep track of. Here the observers are in Component.
    virtual void detach(const int index){
    comps.erase( comps.begin() + index );
    }//We detach using index number of the list of observers.
    virtual void notify(){
...
int main()
{
  int choice = 0, chL = 0, chN = 0, chS = 0, chO = 0;// we use these for choices in the menu.
  int Cno = 0, Dno = 0, Fno = 0, Sno = 0, Ono;// We use these for keeping an counter for College Name, Department Name, Faculty Name, and Student Name.
  bool m = true;// We use this to keep the menu always running.
  std::vector<Composite*> cmps, dmps, fmps, smps;// We use this to create a Dynamic List of different Colleges, Departments, Faculty, and Student records.
  std::vector<Observable*> obs;
  string Dname, Mname, Cname, Fname, Sname, Oname;// We use this string to store names of different Colleges, Departments, Faculty, and Student from client.
  unordered_map<std::string,int> nameC, nameD, nameF, nameS, nameO;// We use this to store the different Colleges, Departments, Faculty, and Student names as the key and their values.
  Composite* composite = new Composite("FS");// We use this object to store our University and as the root of our hierarchy.
  ...
  }else if(choice==4)// Use for Displaying the hierarchy.
  {
      ...
      if(Sno>0){
        ...
        if(chL==1){// Used to subscribe to FS ListServers.
            obs.push_back(new Observable("FS"));
            nameO["FS"]=Ono;// We store the Faculty name in the unorderedmap
            Ono++;
            if(Cno>0){
                for(int i = 0; i< Cno; i++){
                    obs[nameO["FS"]]->attach(cmps[i]);// Adding colleges to FS ListServer.
                }
             }
            ...
            std::cin>>chN;
            if(chN==1){
                obs[nameO["FS"]]->notify();// Display all the subscribed members in FS ListServers.
                chN=0;
            } else{
                chN=0;
            }
            ...
            std::cin>>Cname;
              obs.push_back(new Observable(Cname));
              nameO[Cname]=Ono;
              Ono++;
              if(Dno>0){
                  for(int j = 0; j< Dno; j++){
                    obs[nameO[Cname]]->attach(dmps[j]);// Adding departments to College ListServer.
                  }
              }
            ...
            std::cin>>Dname;
              obs.push_back(new Observable(Dname));
              nameO[Dname]=Ono;
              Ono++;
              if(Fno>0){
                for(int k = 0; k< Fno; k++){
                  obs[nameO[Dname]]->attach(fmps[k]);// Adding faculties to Department ListServer.
                }
              }
              if(Sno>0){
                for(int l = 0; l< Sno; l++){
                    obs[nameO[Dname]]->attach(smps[l]);// Adding students to Department ListServer.
                }
              }
        ...
        std::cout<<"\n Choose the ListServers you want to unsubscribe to: 1) College ListServers 2) Department ListServers";
        std::cin>>chL;
        if(chL==1){//Unsubscribe to College ListServer
